...
class TransformerAgent:
    ...
    def __init__(
        ...
        train_encoder_jointly: bool=False,
        ...
        self.seed = seed
        self.train_encoder_jointly=train_encoder_jointly

        if self.train_encoder_jointly:
            print("🚨 [Strong Baseline Mode] Encoder joint training ENABLED.")
            # 检查配置，确保 model_path 是 None (这步很重要，防止意外加载 Stage 1 权重)
            enc_cfg = transformer_encoder_cfg.representation_transformer
            if hasattr(enc_cfg, 'model_path') and enc_cfg.model_path is not None:
                print(f"⚠️ WARNING: model_path is set to {enc_cfg.model_path} but we are in Baseline mode!")
                print("⚠️ Please set `model_path: null` in your hydra config for a fair comparison!")
        # components
        if self.use_world_model and world_model_cfg is not None:
            ...
            pretrained_dir = os.path.join(pretrained_dir, experiment, f"model_world_seed_{seed}")
        ...
        if is_mlp_encoder_mode:
            ...
            # 计算展平后的总输入维度
            state_dim = 21 # 你的压缩状态维度
            action_dim = self.action_shape[0]
            repr_trans_cfg_dict = dict(repr_trans_cfg)
            sequence_len = repr_trans_cfg_dict.pop('sequence_len', self.seq_len) # 从配置或默认值获取
            
            flattened_input_dim = (sequence_len * state_dim) + \
                                ((sequence_len - 1) * action_dim) + \
                                ((sequence_len - 1) * 1)
            
            # 使用 hydra.utils.instantiate，但【覆盖】env_obs_shape
            self.task_encoder = hydra.utils.instantiate(
                repr_trans_cfg_dict,
                env_obs_shape=[flattened_input_dim] # 关键的覆盖！
            ).to(self.device)
            ...
            # 如果不是 MLP 模式，就执行原来的 Transformer 创建逻辑
        ...
        if self.use_cls_prediction_head and self.prediction_head_cls is not None:
            self._components["prediction_head_cls"] = self.prediction_head_cls
        ...
        for comp_name, component in self._components.items():
            if comp_name == 'transformer_encoder' and self.use_cls_prediction_head:
                if self.train_encoder_jointly:
                    ...
                    component.train(False) # 只有在 Ours (Stage 2) 时才冻结
            else:
                component.train(training)
        ...
        if isinstance(self.task_encoder, FeedForwardEncoder):
            ...
            if disable_grad:
                with torch.no_grad():
                    ...
                    # 2. FeedForwardEncoder.forward 需要一个 MTObs 对象，我们创建一个临时的
                    dummy_mtobs = MTObs(env_obs=mlp_input, task_obs=None, task_info=None)
                    
                    # 3. 调用 encoder，它会直接输出 cls_token
    ...
    def act(
        ...
        with torch.no_grad():
            ...
            if return_tensor:
                return action  # 直接返回 GPU Tensor，不等待，不拷贝！
            else:
                return action.detach().cpu().numpy() # 旧逻辑：发生同步和拷贝

    def select_action(self, states: TensorType, actions: TensorType, rewards: TensorType, task_ids, return_tensor=False) -> np.ndarray:
        return self.act(states, actions, rewards, task_ids, sample=False, return_tensor=return_tensor)

    def sample_action(self, states: TensorType, actions: TensorType, rewards: TensorType, task_ids, return_tensor=False) -> np.ndarray:
        return self.act(states, actions, rewards, task_ids, sample=True, return_tensor=return_tensor)
    ...
    def get_parameters(self, name: str) -> List[torch.nn.parameter.Parameter]: 
        ...
        if name == "actor":
            ...
            # 1. 拿到 Transformer 本体的参数
            params = list(self._components["transformer_encoder"].parameters())
            
            # 2. 【关键判断】如果是 Strong Baseline (联合训练)，就把 Head 也加进来一起练
            # 如果是 Ours (Stage 2)，这里 train_encoder_jointly 是 False，就不会加 Head，保持冻结。
            if self.train_encoder_jointly:
                if self.use_cls_prediction_head and "prediction_head_cls" in self._components:
                    print(f"✅ [Optimizer] Strong Baseline Mode: Adding Prediction Head to optimizer.")
                    params += list(self._components["prediction_head_cls"].parameters())
    ...
    def distill_actor(
        ...
        **kwargs
        ...
        """
        should_disable_grad = not self.train_encoder_jointly
        ...
        if self.use_zeros:
            ...
            task_encoding = self.get_cls_encoding(
                ...
                disable_grad=should_disable_grad, mask=None
            )
    ...
    def evaluate_world_model_total(self, validation_buffer: DistilledReplayBuffer, batch_size: int = 512):
        """
    ...
    def evaluate_world_model(self, validation_buffer: DistilledReplayBuffer, batch_size: int = 512, max_batches: int = 100):
        """
        在验证集上评估 World Model。
        为了速度，默认只随机采样 max_batches 个批次进行估算，而不是遍历全集。
        
        Args:
            max_batches (int): 最大评估批次数。设为 None 则进行全量评估。
        """
        if not self.use_world_model or self.world_model is None:
            print("[Agent-WARN] World model is not configured for this agent.")
            return {}
        ...
        # 2. 【关键】生成随机索引，确保覆盖不同任务/机器人
        # 我们随机打乱所有索引，然后只取我们需要的那一部分
        all_indices = np.random.permutation(buffer_size)
        # 截取前 num_batches * batch_size 个数据
        ...
        with torch.no_grad():
            for i in range(num_batches):
                ...
                # 边界保护 (虽然上面的切片逻辑通常不会越界，但为了 robust)
                if len(batch_idxs) == 0:
                    continue
        ...
        # 计算平均值
        # 注意：如果有 batch 大小不一致的情况，最好按 sample 数加权平均，
        # 但既然我们是随机采样的固定 batch_size，直接除以 num_batches 足够精确。
        if num_batches > 0:
            avg_dynamics_loss = total_dynamics_loss / num_batches
            avg_reward_loss = total_reward_loss / num_batches
            avg_total_loss = total_wm_loss / num_batches
    ...
    def load_world_model(self, model_dir: str, step: Optional[int] = None, load_optimizer: bool = False) -> int:
        ...
        def _latest_step(name: str) -> Optional[Union[int, str]]:
            p = Path(model_dir)
            # 1. 先找所有符合 pattern 的文件
            ...
            steps = []
            for x in cands:
                suffix = x.stem.split("_")[-1]
                # 2. 尝试转 int，如果转不了（比如是 best），就保留原字符串
                if suffix.isdigit():
