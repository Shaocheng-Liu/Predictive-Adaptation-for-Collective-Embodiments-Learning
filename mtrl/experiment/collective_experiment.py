...
class Experiment(checkpointable.Checkpointable):
    def __init__(self, config: ConfigType, experiment_id: str = "0"):
        ...
        modes_without_envs = ['train_world_model', 'evaluate_world_model']
        if self.config.experiment.mode in modes_without_envs:
            print(f"[Optimization] Skipping environment creation for mode: {self.config.experiment.mode}")
            self.envs = {}
            self.env_metadata = self._build_minimal_metadata()
        else:
        ...
        if self.config.experiment.mode == 'train_worker':
            ...
            print(f"Loading collective agent from: {self.col_model_dir}")
            self.col_start_step = self.col_agent.load_latest_step(model_dir=self.col_model_dir)
            ...
            self.replay_buffer_distill = [hydra.utils.instantiate(
                self.config.replay_buffer.transformer_col_replay_buffer,
                device=self.device,
                capacity=self.config.replay_buffer.eval_capacity,
                env_obs_shape=env_obs_shape,
                task_obs_shape=(1,),
                action_shape=action_shape,
                seq_len=self.seq_len,
            ...
            self.col_model_dir = utils.make_dir(
                os.path.join(self.config.setup.save_dir, "model_dir",self.config.experiment.experiment, f"model_world_seed_{self.config.setup.seed}")
            )

            # 3) 训练数据：聚合 train split 的 collective buffers（与 distill_collective_transformer 相同套路）
            self.col_buffer_loc = utils.make_dir(
                os.path.join(self.config.setup.save_dir, "buffer/collective_buffer_3/train")
            )
            self.distill_buffer_names = os.listdir(self.col_buffer_loc)
            ...
            buffer_dirs = [
                utils.make_dir(os.path.join(self.config.setup.save_dir, f"buffer/collective_buffer_3/train/{i}"))
                for i in self.distill_buffer_names
            ]
            ...
            self.buffer_dir_val = [utils.make_dir(
                os.path.join(self.config.setup.save_dir, f"buffer/collective_buffer_3/validation/{i}")
            ) for i in self.distill_buffer_names]
            ...
            # 2. 实例化 Distill Replay Buffer
            # 注意：这里使用 col_replay_buffer (通常是简单的数组 buffer)，而不是 transformer buffer
            self.replay_buffer_distill = [hydra.utils.instantiate(
                self.config.replay_buffer.col_replay_buffer,
                device=self.device,
                env_obs_shape=env_obs_shape,
    ...
    def _build_minimal_metadata(self) -> EnvMetaDataType:
        """Build minimal environment metadata without creating actual environments.
        
        This is used for modes like train_world_model and evaluate_world_model that
        don't need actual environment instances but still need basic metadata like
        action_space and observation_space.
        
        Returns:
            EnvMetaDataType: Dictionary containing minimal environment metadata.
        """
        # Import here to avoid issues if metaworld is not available
        import hydra
        from gymnasium import spaces
        
        # Create benchmark to get task information
        benchmark = hydra.utils.instantiate(self.config.env.benchmark)
        
        # Extract task information without creating environments
        train_tasks = benchmark._train_tasks
        task_names = list(set([task.env_name for task in train_tasks]))
        
        # For metaworld environments, we know the standard spaces
        # Action space: 4D continuous actions in [-1, 1]
        action_space = spaces.Box(low=-1.0, high=1.0, shape=(4,), dtype=np.float32)
        
        # Observation space: 39D for standard metaworld (can be overridden by config)
        obs_dim = 39  # Standard metaworld observation dimension
        env_obs_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(obs_dim,), dtype=np.float32
        )
        
        # Max episode steps for metaworld
        max_episode_steps = 400
        
        metadata: EnvMetaDataType = {
            "env_obs_space": env_obs_space,
            "action_space": action_space,
            "ordered_task_list": task_names,
