# mtrl/env/real_envs/window_close_env.py
"""
KUKA Window Close 任务环境

任务: 关闭滑动窗户
成功条件: 窗户把手 X 位置与目标距离 < 0.05m

基于 MetaWorld sawyer_window_close_v2.py 的 Reward 逻辑，
适配 real_envs/base_env.py 的真机接口风格。

Apriltag 遮挡策略 (2024):
    由于推窗过程中机械臂会遮挡 Apriltag，采用以下策略（与 window_open 相同）:
    1. reset 时读取一次 Apriltag 作为把手初始位置，之后不再实时读取
    2. 分两阶段计算 reward:
...
try:
    ...
    print("[window_close_env] Warning: MetaWorld not available, using local fallback")
...
class KukaWindowCloseEnv(KukaBaseRealEnv):
    """
    Window Close 任务环境
    ...
    MetaWorld 中 state[4:7] 的含义:
        _get_pos_objects() = _get_site_pos("handleCloseStart")
        handleCloseStart 在 windowb_a 本体坐标系中位于 [0.01, -0.095, 0]
        由于 handle 和 site 在同一刚体上，偏移恒定:
        _get_pos_objects ≈ handle_position + [0.01, -0.095, 0] (sim 坐标, 相对于窗体)
        
        handle_position = body + [qpos, 0, 0], 初始 qpos=0.2
    ...
    Apriltag 遮挡处理:
        ...
        - step() 中根据 TCP 是否靠近把手来切换 reward 计算模式
          （与 window_open 采用完全相同的策略）:
          Phase 1: TCP 远离把手 → 使用固定的初始 object_position
          Phase 2: TCP 靠近把手 → 根据 TCP 位移估算窗户位置
    
    使用方法:
        env = KukaWindowCloseEnv()
        env.set_handle_position(np.array([0.85, 0.0, 0.2]))
    ...
    CONTACT_THRESHOLD = 0.04  # 4cm
    
    # MetaWorld: _get_pos_objects = _get_site_pos("handleCloseStart")
    #            handleCloseStart at [0.01, -0.095, 0] in windowb_a local frame (sim coords)
    #            handle position ≈ body + [qpos, 0, 0]
    #            _get_pos_objects - handle ≈ [0.01, -0.095, 0]  (constant, sim coords)
    # 转换到真实坐标系: R_real_sim * [0.01, -0.095, 0] = [-0.095, -0.01, 0]
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(task_name="window-close-v2", **kwargs)
        
        # 最大滑动距离
        self.maxPullDist = 0.2
        
        # 默认把手位置（真实坐标系，打开状态）
        # Window-close: 把手初始位置在打开状态，需要滑向关闭位置
        self._handle_position = np.array([0.85, 0.0, 0.2])
        ...
        self._simulate_mujoco_bug = False
    ...
    def reset(self) -> np.ndarray:
        ...
        """
        self._simulate_mujoco_bug = True
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        # 2. 调用父类 step（执行动作、获取观测、计算reward等）
        # 注意: 由于 track_object_realtime=False，父类不会读取 Apriltag
        obs, reward, done, info = super().step(action)
        
        # ============================================================
        # 基于 TCP 位移估算窗户位置（与 window_open 相同策略）
        # ============================================================
        tcp = self.ee_current_position.copy()
        tcp_to_obj = float(np.linalg.norm(self.obj_init_pos - tcp))
        ...
        if not self._contact_made:
            ...
            if tcp_to_obj <= self.CONTACT_THRESHOLD:
                ...
                print(f"[KukaWindowCloseEnv] Contact made! TCP={tcp}, obj_init={self.obj_init_pos}")
        ...
        # 添加调试信息
        info['contact_made'] = self._contact_made
        
        return obs, reward, done, info
    
    def compute_reward(self, action: np.ndarray, obs_dict: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
        """
        计算奖励 - 对齐 MetaWorld V2 Window Close
        
        奖励组成:
        1. reach: 夹爪接近把手的奖励
        2. in_place: 把手接近目标（关闭）位置的奖励
        
        Apriltag 遮挡适配（与 window_open 相同策略）:
        ...
        特点：Window Close 沿 real Y 轴 (= -sim X) 方向移动
        """
        # 获取位置（真实坐标系）
        tcp = self.ee_current_position
        obj = self.object_position  # 把手当前位置（固定值或 TCP 估算值）
        target = self.goal.copy()   # 把手目标位置（关闭后）
        
        # 计算把手到目标的距离（沿窗户滑动轴: real Y = -sim X）
        # 窗户在 sim X 方向滑动，对应 real Y 方向 (axis [1])
        target_to_obj = float(abs(obj[1] - target[1]))
        
        # 初始距离
        target_to_obj_init = float(abs(self.window_handle_pos_init[1] - target[1]))
    ...
    def _get_pos_objects(self, color: str = 'green') -> np.ndarray:
        ...
        不再从相机读取（会被遮挡），直接返回当前的 object_position
        """
        if self._simulate_mujoco_bug:
            # 仿真中的 Bug 表现为：
            # 真实位置已经加了 qpos=0.2 (sim X 方向 = -real Y 方向)，
            # 但观测读数还是旧的 (qpos=0)
            # Bug 差异: sim [-0.2, 0, 0] → real [0, +0.2, 0]
            buggy_pos = self.object_position.copy()
            buggy_pos[1] += 0.2
            return buggy_pos
        return self.object_position.copy()
    
    def _get_object_quat(self) -> np.ndarray:
        """Window Close 四元数 - 与 MetaWorld 一致返回 zeros(4)"""
        return np.zeros(4)