...
class COL_MT5(Benchmark):
    ...
    def __init__(self, env_name, seed=None):
        ...
        self._train_classes = OrderedDict([(env_name+f"-{i}", cls) for i in range(5)])
        
        train_kwargs = {env_name+f"-{i}": args_kwargs for i in range(5)}
...
# Auto-apply robot configuration patches when this module is imported
# This ensures patches are applied in ALL processes including subprocesses
try:
    from metaworld.envs.robot_utils import ensure_robot_config
    ensure_robot_config()
except Exception as e:
    # Don't fail if robot_utils is not available
    import os
    if os.environ.get("METAWORLD_ROBOT_TYPE"):
        print(f"[RobotConfig] Warning: Could not auto-apply patches: {e}")
