...
class TFDataset(Dataset):
    ...
    def save(self, file_path, start_chunk):
        ...
        for i in range(num_chunks):
            ...
            # 修正：使用 save_dir 而不是未定义的 safe_path
            if save_dir:
...
if __name__ == "__main__":
    ...
    path_data = os.path.join(project_root, "Transformer_RNN/dataset_z1/")
    # 根输出目录
    base_safe_path = os.path.join(project_root, 'Transformer_RNN/decision_tf_dataset_z1/')
    subdicts = ['train', 'validation'] # 去掉斜杠，方便路径拼接
    ...
    # 第一步：预扫描所有文件夹，建立统一的 robot2id 映射
    # 确保训练集和测试集的 Robot ID 编码完全一致
    robot2id, next_rid = {}, 0
    all_robots = []
    for sub in subdicts:
    ...
    for sub in subdicts:
        ...
        # 物理隔离的输出路径：例如 decision_tf_dataset/train/
