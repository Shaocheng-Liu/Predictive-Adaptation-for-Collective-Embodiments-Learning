...
def universal_train_loop(encoder,
    ...
    for batch in data_loader:
        ...
        # --- Sergey 的探针诊断 (DEBUG START) ---
        if counter == 0: # 只在第一个 step 打印，防止刷屏
            print("\n" + "="*30)
            print("Sergey's GPU Diagnostic Check:")
            print(f"1. Pytorch 认为的 device 变量: {device}")
            print(f"2. 输入 Tensor (states) 所在的设备: {states.device}")
            print(f"3. 模型 Encoder 参数所在的设备: {next(encoder.parameters()).device}")
            
            # 强制检查：如果不一致直接报错
            assert str(states.device).startswith("cuda"), "❌ 严重错误：数据还在 CPU 上！"
            assert str(next(encoder.parameters()).device).startswith("cuda"), "❌ 严重错误：模型还在 CPU 上！"
            
            print("✅ 通过检查：模型和数据都在 GPU (CUDA) 上运行。")
            print("="*30 + "\n")
        # --- Sergey 的探针诊断 (DEBUG END) ---
...
bnpy_save_dir = project_root + '/Transformer_RNN/bnpy_save_task_dyn_true_3_seed_3'
...
use_dynamic_loss = True  # Enable dynamic loss to predict next state (world model style)
...
embedding_num = 20000
model_path = 'Transformer_RNN/checkpoints_task_dyn_true_3_seed_3/representation_cls_transformer_checkpoint.pth'
base_dir = 'Transformer_RNN/decision_tf_dataset_3/train/'
...
val_base_dir = 'Transformer_RNN/decision_tf_dataset_3/validation/'
...
embeddings_path = 'Transformer_RNN/embedding_log_task_dyn_true_3_seed_3' \
'/emb.pth'
